...
class Job {
    ...
    static async create(data) {
        ...
        INSERT INTO jobs (title, salary, equity, company_handle)
         VALUES ($1, $2, $3, $4)
         RETURNING id, title, salary, equity, company_handle AS             
         "companyHandle"`,
         [
            data.title,
            data.equity,
    ...
    static async get(id) {
        const jobRes = await db.query(`SELECT
                            id,
                            title,
                            salary,
                            equity,
                ...
                const companiesRes = await db.query(`SELECT
                            handle,
                            name,
                            description,
    ...
    static async update(id, data) {
        const { setCols, values } = sqlForPartialUpdate(
            data,
            {});
        const inVarIdx = "$" + (values.length + 1);
